...
public class Predetermine {
	...
	public ModelAndView tolist(HttpServletRequest request, Integer currentPage,
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (shiJianCha>=0) {
				...
				int remind = list.get(i).getRemind();
				if( remind != 1) {
	...
	public ModelAndView delete(String id){
		...
		for (int i = 0; i < FenGe.length; i++) {
			int predetermineID = Integer.parseInt(FenGe[i]);
			PredeterminePo pdm = predetermineService.findById(predetermineID);
			roomSetPo.setId(pdm.getRoomID());
			roomSetService.updateByIdToRoomState(roomSetPo);
			this.predetermineService.deleteById(predetermineID);
		}
		mv=new ModelAndView("redirect:/Predetermine/tolist.do");
		return mv;
	}
